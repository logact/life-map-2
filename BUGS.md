...
### B3 — Node inspector unreachable (no way to rename/edit nodes)
`src/app/map.tsx:3049–3193`, `:1683`, `:1141`

`setInspectorNodeId` is only ever called with `null`. Either the feature ships or the ~150 lines go.

- **Fix (recommended):** add an "Edit" action to the node sheet (`:2464`) that seeds `inspectorDraft` from the node and calls `setInspectorNodeId(node.id)`. Alternative: delete the block, `saveInspector`, `inspectorDraft`, and the `inspector*` styles.
- **Verify:** double-tap node → Edit → change title → visible on canvas after Save.
